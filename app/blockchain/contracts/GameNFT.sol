...
/**
 * @title GameNFT
 * @dev ERC-721 NFT with gameplay bonuses for Tactizen
 * @notice These NFTs are fully transferable and provide in-game benefits
 */
contract GameNFT is ERC721, ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    // Treasury address for burned NFTs (can re-mint for loot drops)
    ...
    // Events
    event NFTMinted(address indexed to, uint256 indexed tokenId, string nftType, string category, uint8 tier);
    ...
    constructor(address _treasury) ERC721("Tactizen Game NFT", "TACTIZEN") Ownable(msg.sender) {
    ...
    function mintNFT(
        ...
        emit NFTMinted(to, tokenId, nftType, category, tier);

        return tokenId;
    }

    /**
     * @dev Burn NFT and send to treasury
