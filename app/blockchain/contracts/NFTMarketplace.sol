 ...
 * @dev P2P NFT marketplace with escrow for Tactizen
...
contract NFTMarketplace is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Force clear an orphaned listing (only owner)
     * @param tokenId Token ID with orphaned listing
     * @notice Use this when a listing exists but NFT is not in the contract
     */
    function forceClearListing(uint256 tokenId) external onlyOwner {
        Listing storage listing = listings[tokenId];
        require(listing.active, "Listing not active");

        // Mark as inactive and remove from active listings
        listing.active = false;
        _removeFromActiveListings(tokenId);
