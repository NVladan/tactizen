...
contract GameNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor(address _zenToken, address _treasury) ERC721("Tactizen Game NFT", "TACGAME") Ownable(msg.sender) {
    ...
    /**
     * @dev Upgrade NFT by burning 3 of same tier AND same category to create 1 of next tier
     * @param burnTokenIds Array of 3 token IDs to burn (must be same tier AND same category)
    ...
    function upgradeNFT(
        ...
        // Verify all NFTs are owned by sender and same tier AND same category
        uint8 tier = nftMetadata[burnTokenIds[0]].tier;
        string memory nftType = nftMetadata[burnTokenIds[0]].nftType;
        string memory category = nftMetadata[burnTokenIds[0]].category;
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            require(
                keccak256(bytes(nftMetadata[burnTokenIds[i]].nftType)) == keccak256(bytes(nftType)),
                "All NFTs must be same type"
            );
            require(
                keccak256(bytes(nftMetadata[burnTokenIds[i]].category)) == keccak256(bytes(category)),
                "All NFTs must be same category"
            );
        ...
        // Mint new NFT of next tier with SAME type and category
